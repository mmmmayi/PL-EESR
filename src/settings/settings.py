...
class ComputingSettings(AbstractSettings):
    ...
    network_dataloader_workers: int = 2
...
class NetworkSettings(AbstractSettings):
    ...
    eer_stop_epochs: int = 10  # Stop training if EER does not improve in this many epochs
    ...
    print_interval: int = 100  # How often to print traninig loss
    accuracy_print_interval: int = 8  # Evaluate accuracy every nth print (Too small value will affect speed)
    extraction_print_interval: int = 1000  # Print interval in embedding extraction (in batches)

    optimizer: str = 'Adadelta'

    max_consecutive_lr_updates = 2
    initial_learning_rate1: float = 0.01
    initial_learning_rate2: float = 0.1
    ...
    network_class: str = 'asvtorch.src.networks.architectures.StandardNet_Mask'
