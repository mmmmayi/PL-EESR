...
class UtteranceList:
    ...
    def sort(self,reverse=True):
        ...
        for utterance in self.utterances:
            utts_temp.append([utterance,utterance.n_signal])
        sort_utts = sorted(utts_temp,key=lambda info:int(info[1]),reverse=reverse)
    ...
    def remove_short_utterances(self, min_length: int):
        ## need to edit if add vad function
        orig_count = len(self.utterances)
#        min_frame2sec = ((min_length-1)*10+25)/1000
#        min_n = min_frame2sec*16000
        #for x in self.utterances:
            #print('x.n_signal:{}'.format(x.n_signal))
        self.utterances = [x for x in self.utterances if x.n_signal >= min_length]
    ...
    def remove_speakers_with_few_utterances(self, min_utt_count: int):
        ...
        for utt in self.utterances:
            if 'noise' in utt.utt_id or 'music' in utt.utt_id or 'babble' in utt.utt_id or 'rev' in utt.utt_id:
                continue
            else:
    ...
    def convert_labels_to_numeric(self):
        ...
        for index, label in enumerate(labels):
            index_dict[label].append(index) #{'id0001': [0], 'id0002': [1]}
        temp = open('/data07/mayi/code/asvtorch/asvtorch/recipes/sitw/xvector/sitw_outputs/datasets/voxceleb2_mask_aug/lists/temp_pre','r')
        line = temp.readline()
        align = {}
        test = open('/data07/mayi/code/asvtorch/asvtorch/recipes/sitw/xvector/sitw_outputs/datasets/voxceleb2_mask_aug/lists/test','w')
        while line:
            parts=line.split(' ')
        ...
        for label, key in enumerate(index_dict):
            ...
            for index in index_dict[key]:
                if i ==0:
                    number = align[self.utterances[index].spk_id].strip('\n')
                    number = int(number)
                    test.write(self.utterances[index].spk_id+' '+str(number)+'\n')
                    i=i+1
                #print('test for convert_labels_to_numeric:',label)
                #print(self.utterances[index].spk_id+' '+str(label))
                self.utterances[index].spk_id = number
            #temp.write()
