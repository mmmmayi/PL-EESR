...
def train_network(training_data: UtteranceList,mean, std, mean_lps, std_lps, writer, resume_epoch: int = 0):
    ...
    scheduler = MultiStepLR(optimizer=optimizer, milestones=[6, 12, 18, 24], gamma=0.8)
    ...
    net.train()
    net_training_loss = torch.Tensor([torch.finfo().max])
    net_consecutive_lr_updates = torch.LongTensor([0])
    ...
    for epoch in range(1, settings.epochs_per_train_call + 1):
        ...
        #i = -1
        #for _ in range(math.ceil(Settings().network.utts_per_speaker_in_epoch * 4682 / Settings().network.minibatch_size)):
        for i, batch in enumerate(training_dataloader):
            features, lps, speaker_labels = batch
            ...
            o_lps, o_spk_id = net(features)
            print('o_spk_id shape:{}, speaker_labels shape:{}'.format(o_spk_id.shape, speaker_labels.shape))
            loss1 = (criterion1(o_spk_id, speaker_labels)) / settings.optimizer_step_interval
            
            loss2 = criterion2(o_lps, lps)
            loss = loss1+loss2
                 #print('{} debug loss: {}'.format(i,loss.item()))
            loss.backward()
            ...
            if i % settings.print_interval == settings.print_interval - 1:
                writer.add_scalar('loss1', loss1,i+(epoch + resume_epoch-1)*len(training_dataloader))
                writer.add_scalar('loss2', loss2,i+(epoch + resume_epoch-1)*len(training_dataloader))
                writer.add_scalar('loss', loss,i+(epoch + resume_epoch-1)*len(training_dataloader))
                ...
                if i % (settings.print_interval * settings.accuracy_print_interval) == settings.print_interval * settings.accuracy_print_interval - 1:
                    ...
                    val_data = compute_losses_and_accuracies((validation_dataloader_1, validation_dataloader_2), net, criterion1, criterion2, mean, std, mean_lps, std_lps)
                    output = 'Epoch {}, Time: {:.0f} s, Batch {}/{}, lr: {:.6f}, train-loss: {:.3f}, subset-cross-entropy: {:.3f}, val-cross-entropy: {:.3f}, subset-MSEloss: {:.3f}, val-MSEloss: {:.3f}, subset-acc: {:.3f} val-acc: {:.3f}'.format(epoch + resume_epoch, time.time() - start_time, i + 1, len(training_dataloader), current_learning_rate, training_loss / settings.print_interval, val_data[0][0], val_data[1][0],val_data[0][1], val_data[1][1], val_data[0][2], val_data[1][2])
                    torch.cuda.empty_cache()
                else:
        ...
        prev_loss = net_training_loss.item()
...
#                param_group['lr'] = param_group['lr']/2
            net_consecutive_lr_updates[0] += 1
#            print('Because loss change {:.2f}% <= {:.2f}%, the learning rate is halved: {} --> {}'.format(loss_change*100, Settings().network.min_loss_change_ratio*100, optimizer.param_groups[0]['lr'] * 2, optimizer.param_groups[0]['lr']))
            print('Consecutive LR updates: {}'.format(net_consecutive_lr_updates[0]))
        else:
            net_consecutive_lr_updates[0] = 0

        net_training_loss[0] = current_loss
        network_io.save_state(output_filename, epoch + resume_epoch, net, optimizer)

        if net_consecutive_lr_updates[0] >= Settings().network.max_consecutive_lr_updates:
...
def _init_optimizer(net, settings):
    ...
    if settings.optimizer == 'Adadelta':
        return optim.Adadelta(params, lr=settings.initial_learning_rate)
